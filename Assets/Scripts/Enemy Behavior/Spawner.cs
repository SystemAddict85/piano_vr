...
public class Spawner : MonoBehaviour
{
    ...
    void createPool(int size = 0)
    {
        ...
        {
            GameObject obj = m_pathingBox;
            obj.SetActive(false);
            obj = Instantiate(m_pathingBox);
            obj.transform.parent = this.transform;
            obj.transform.position = transform.position;

            IdlePath idleRef = obj.GetComponent<IdlePath>();
            EnemyManager.Instance.ApplyIdleBoxSettings(ref idleRef);
    ...
    void createIdleEnemies(int size = 0)
    {
        for (int i = 0; i < size; ++i)
        {
            ...
            EnemyManager.Instance.ApplyEnemySettings(ref enemyRef);
            //enemyRef.m_reg_speed = 1.0f;
